...
string whois(const void* address, bool include_offset = false) {
    Dl_info info;
    if (!dladdr(address, &info)) { if(include_offset){ char b[64]; snprintf(b,sizeof(b),"[!dladdr](0x%lx)",(unsigned long)(uintptr_t)address); return std::string(b);} return "[!dladdr]"; }
    ...
    { char b[256]; snprintf(b,sizeof(b),"%s(0x%lx)", libname.c_str(), (unsigned long)offset); return std::string(b); }
}

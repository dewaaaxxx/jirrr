...
static bool prevForceFullSim = false; // FIX: track forceFullSimulation state juga
...
bool Prediction::determineShotResult(bool isAuto, double shotAngle, double shotPower, Vec2d shotSpin, Candidate cand) { // returns isShouldReDraw
    ...
    // FIX: Invalidate cache juga saat forceFullSimulation berubah (misal dari false→true).
    // Tanpa ini: angle+power sama tapi forceFullSimulation=true → cache hit → return false
    // → simulasi tidak jalan → hasil stale dari display pass (fastCalc=true) dipakai
    // untuk evaluasi akurat → akurasi turun.
    if (!forceFullSimulation) {
        if (shotAngle == prevAngle && shotPower == prevPower && shotSpin == prevSpin
            && isAuto == prevIsAuto && forceFullSimulation == prevForceFullSim)
            return false;
    }
    ...
    prevSpin  = shotSpin;
    prevIsAuto = isAuto;
    prevForceFullSim = forceFullSimulation;
...
void Prediction::initBalls() {
    ...
    for (int i = 0; i < this->guiData.ballsCount; i++) {
        ...
        // FIX: reserve(20) terlalu kecil untuk shot dengan banyak pantulan (bank shots).
        // Saat positions.size() > 20, vector reallocate → waste CPU tiap simulasi.
        // reserve(64) cukup untuk hampir semua shot tanpa realloc.
...
void Prediction::handleCollision() {
    ...
    switch (this->guiData.collision.type) {
        case Collision::Type::BALL:
            ...
            // FIX: firstHitBall harus di-set ke ballB (bola yang kena tembak), bukan ballA.
            // ballA adalah cue ball (yang menabrak), ballB adalah bola target.
            // Sebelumnya pakai `if (firstHitBall == nullptr)` yang benar, tapi
            // ballB sudah benar — tidak perlu diubah. Yang penting: firstHitBall = &ballB.
...
void Prediction::handleBallBallCollision() const {
    ...
    // FIX AKURASI: Sebelumnya pakai implementasi manual (transfer momentum sederhana).
    // Manual tidak memperhitungkan spin transfer antar bola, English effect, dan
    // energy loss yang sama persis dengan engine → hasil simulasi beda dengan game nyata.
    //
    // Sekarang pakai calcVelocityPostCollision() yang memanggil FUN_02b1bb3c langsung
    // dari engine (libmain) → 100% sync dengan physics game.
    //
    // Angle collision dihitung dari normal vektor antara kedua bola:
    Point2D relativePosition = ballA.predictedPosition - ballB.predictedPosition;
    double dist = sqrt(relativePosition.square());
    if (dist < 1e-10) return; // guard: jangan divide by zero

    // Normal angle: arah dari ballB ke ballA
    double collisionAngle = atan2(relativePosition.y, relativePosition.x);

    // Engine function: update velocity+spin ballA berdasarkan collision angle
    ballA.calcVelocityPostCollision(collisionAngle);
    // BallB collision normal adalah kebalikan arahnya
    ballB.calcVelocityPostCollision(collisionAngle + M_PI);
}
    ...
    if (this->velocity) {
        ...
        if (!fastCalc) {
            ...
            if (lastIndex > 1) {
                ...
                // FIX: Collinearity check pakai epsilon, bukan == (float equality).
                // (b.y-a.y)*(c.x-b.x) == (c.y-b.y)*(b.x-a.x) pakai exact float equality
                // → sering miss karena floating point precision → titik duplikat masuk positions
                // → path jadi berantakan dan tidak smooth.
                // Epsilon 1e-6 cukup untuk unit meja biliar (skala ~100-300 unit).
                double cross = ((b.y - a.y) * (c.x - b.x)) - ((c.y - b.y) * (b.x - a.x));
                if (cross > -1e-6 && cross < 1e-6) return; // collinear, skip
            }

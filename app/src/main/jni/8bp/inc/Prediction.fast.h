...
struct Prediction {
    ...
    // forceFullSimulation=true: simulasi berjalan penuh tanpa early-return
    // saat firstHit ditemukan. Wajib untuk deteksi scratch dan 8-ball masuk
    // yang benar — tanpa ini cue ball belum selesai disimulasikan saat check.
...
bool Prediction::determineShotResult(bool isAuto, double shotAngle, double shotPower, Vec2d shotSpin, Candidate cand) {
    // Skip cache saat forceFullSimulation aktif — butuh hasil fresh setiap call
    ...
    // forceFullSimulation=true → fastCalc=false → simulasi penuh (tidak early-return)
        ...
        do {
            ...
            if (this->guiData.collision.valid) {
                ...
                if (this->guiData.collision.firstHitBall != nullptr && this->m_candidate.idx != -1) {
                    ...
                    // forceFullSimulation=true: JANGAN early return — simulasi harus
                    // berjalan sampai semua bola berhenti supaya scratch terdeteksi.

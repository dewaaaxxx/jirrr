...
struct Prediction {
    ...
    // forceFullSimulation: kalau true, fastCalc=false → simulasi penuh.
    // Di-set true oleh caller sebelum determineShotResult untuk evaluasi kritis
    // (scratch detection, safety check), lalu di-set false kembali setelahnya.
...
bool Prediction::determineShotResult(bool isAuto, double shotAngle, double shotPower, Vec2d shotSpin, Candidate cand) { // returns isShouldReDraw
    // FIX: skip cache saat forceFullSimulation=true (butuh fresh result),
    // dan include cand.idx dalam cache key supaya kandidat berbeda tidak tabrakan cache.
    ...
    // FIX: forceFullSimulation override fastCalc supaya simulasi penuh dijalankan
    // ketika diperlukan untuk deteksi scratch dan safety check yang akurat.

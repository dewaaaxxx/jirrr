...
namespace AutoPlay {
    ...
    void Shoot(double angle, double power = 0.f) {
        ...
        // Cek apakah meja ini wajib nominasi untuk giliran ini
        ...
        if (mustNominate && g_CurrentCandidate.idx != -1) {
            // Nominasi diperlukan kalau pocket yang sudah dipilih game (currentNominated)
            // berbeda dengan pocket yang kita inginkan (g_CurrentCandidate.pocketIndex).
            // currentNominated >= 6 artinya belum ada nominasi → wajib nominasi.
            if (currentNominated != g_CurrentCandidate.pocketIndex) {
                nominating = true;
            }
        }
        ...
        //    else if (scan == PRECISION) ScanPrecision(0.005f);
            ...
            if (nominationFrameCounter > 20 && !buttonClicker.Active) {
                ...
                if (nominatedPocket == g_CurrentCandidate.pocketIndex) {
                    ...
                    // Nominasi belum dikonfirmasi. Retry click setiap 30 frame.
                    // Kalau sudah 5x retry (150 frame ~2.5 detik) dan masih gagal,
                    // batalkan dan scan ulang — pocket mungkin tidak valid.

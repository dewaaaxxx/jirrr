    ...
    //    AutoPlay::UpdateScanMode();
        ...
        int lineStyle = persistent_int["iLineStyle"];
         ...
         if (persistent_bool[O("bESP_DrawPredictionLine")]) {
            ...
            // Helper: draw dotted/dashed line (used for stripe balls)
            auto AddDottedLine = [&](ImVec2 a, ImVec2 b, ImU32 col, float thickness, float dotLen = 9.f, float gapLen = 9.f) {
                ...
                while (t < dist) {
                    ...
                    if (drawing)
                        draw->AddLine(ImVec2(a.x + nx*t, a.y + ny*t), ImVec2(a.x + nx*end, a.y + ny*end), col, thickness);
                    t = end;
                    drawing = !drawing;
                }
            };
     ...
     /*   if (persistent_bool[O("bESP_DrawPredictionLine")]) {
            ...
            if (lineThick < 0.5f) lineThick = 2.0f;
            for (int i = 0; i < gPrediction->guiData.ballsCount; i++) {
                auto& ball = gPrediction->guiData.balls[i];
                if (ball.initialPosition != ball.predictedPosition) {
                    ImVec2 lastPos{};
                    for (int j = 1; j < ball.positions.size(); j++) {
                        ...
                        if (lastPos.x || lastPos.y) {
                            if (lineStyle == 1) {
                                ...
                                if (steps < 1) steps = 1;
                                float dotRadius = lineThick * 0.5f;
                                for (int s = 0; s <= steps; s++) {
                                    float k = (float)s / (float)steps;
                                    ImVec2 p(lastPos.x + dx*k, lastPos.y + dy*k);
                                    draw->AddCircleFilled(p, dotRadius, getCol(i));
                                    draw->AddCircleFilled(p, dotRadius * 0.5f, IM_COL32(255,255,255,(int)(predA*255)));
                                }
                            } else {
                                draw->AddLine(lastPos, point, getCol(i), lineThick);
                            }
                        }
                        lastPos = point;
                    }
